import { Play, Heart, Info } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export default function OverviewHeader({ movie }: { movie: any }) {
  const navigate = useNavigate();
  return (
    <div className="flex-1 relative w-full h-[400px] sm:h-[500px] md:h-[600px] lg:h-screen">
      <div className="relative rounded-lg overflow-hidden h-full">
        <img
          src={movie.image || '/placeholder.svg'}
          alt={movie.title}
          className="absolute inset-0 w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/80 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950 to-transparent" />
      </div>

      <div className="flex flex-col lg:flex-row gap-1 w-full absolute inset-0 justify-end top-[65%] sm:top-[55%] md:top-[60%] -translate-y-0 sm:-translate-y-1/2 px-4 sm:px-6 md:px-10">
        {/* Content Overlay */}
        <div className="inset-0 w-full lg:w-1/2 flex flex-col p-4 sm:p-6 md:p-10 -translate-y-0 sm:-translate-y-1/2 lg:ml-2">
          <div className="mb-3 sm:mb-4 md:mb-6">
            <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-1 sm:mb-2 leading-tight text-balance">
              {movie.vietnameseTitle}
            </h2>
            <p className="text-amber-400 font-semibold text-sm sm:text-base">
              {movie.title}
            </p>
          </div>

          {/* Info Badges */}
          <div className="flex flex-wrap gap-2 sm:gap-3 mb-3 sm:mb-4 md:mb-6">
            <div className="flex items-center gap-1 sm:gap-2 bg-slate-800/80 px-2 sm:px-3 py-1 sm:py-1.5 rounded border border-slate-700">
              <span className="text-xs text-slate-300">IMDb</span>
              <span className="font-bold text-white text-xs sm:text-sm">
                {movie.rating}
              </span>
            </div>
            <div className="bg-slate-800/80 px-2 sm:px-3 py-1 sm:py-1.5 rounded border border-slate-700 text-xs sm:text-sm font-semibold text-white">
              {movie.ageRating}
            </div>
            <div className="bg-slate-800/80 px-2 sm:px-3 py-1 sm:py-1.5 rounded border border-slate-700 text-xs sm:text-sm font-semibold text-white">
              {movie.year}
            </div>
            <div className="bg-slate-800/80 px-2 sm:px-3 py-1 sm:py-1.5 rounded border border-slate-700 text-xs sm:text-sm font-semibold text-white">
              {movie.duration}
            </div>
          </div>

          {/* Genres */}
          <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4 md:mb-6">
            {movie.genres?.slice(0, 3).map((genre: any) => (
              <span
                key={genre}
                className="text-xs bg-slate-700/50 text-slate-200 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded"
              >
                {genre}
              </span>
            ))}
          </div>
          {/* Action Buttons */}
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            <button
              onClick={() => {
                navigate(`/movie/${movie.id}`);
              }}
              className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full bg-amber-400 hover:bg-amber-500 flex items-center justify-center transition-colors shadow-lg"
            >
              <Play className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-slate-950 fill-slate-950" />
            </button>
            <button className="p-2 sm:p-2.5 md:p-3 rounded-full border-2 border-slate-600 hover:border-white text-slate-300 hover:text-white transition-colors">
              <Heart className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
            </button>
            <button className="p-2 sm:p-2.5 md:p-3 rounded-full border-2 border-slate-600 hover:border-white text-slate-300 hover:text-white transition-colors">
              <Info className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
            </button>
          </div>
        </div>

        <div className="hidden lg:block rounded-lg w-full lg:w-[60%] h-[300px] md:h-[400px] lg:h-[500px] -translate-y-32 justify-between bg-gray-900 p-2 sm:p-3 md:p-4 border-4 border-red-400 blink-border lg:mr-10">
          <img
            src={movie.image || '/placeholder.svg'}
            alt={movie.title}
            className="w-full h-full object-cover"
          />
        </div>
      </div>
    </div>
  );
}
